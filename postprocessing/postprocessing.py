...
def main():
    ...
    if not os.path.exists(
        paths['outputs']['figures']['interactive_parallel_1']
    ):
        ...
        exp = postmocot.viz.interactive_parallel(df_front)
        exp.to_html(paths['outputs']['figures']['interactive_parallel_1'])

    # Interactive parallel plot 2
    if not os.path.exists(
        paths['outputs']['figures']['interactive_parallel_2']
    ):
        objective_names = pd.read_csv(
            paths['inputs']['objectives']
        ).columns.tolist()
    ...
    if not os.path.exists(paths['outputs']['figures']['improvements_1']):
        ...
        # Front animation
        if not os.path.exists(
            paths['outputs']['figures']['front_animation_1']
        ):
            runtime.plot_fronts(
                paths['outputs']['figures']['front_animation_dir']
            )
            # Requires imagemagick http://www.imagemagick.org/script/download.php  # noqa
            os.system("magick convert -delay 10 -loop 0 io/outputs/figures/front_animation/*.png {}".format(paths['outputs']['figures']['front_animation_1']))  # noqa
    ...
    if not os.path.exists(paths['outputs']['figures']['improvements_2']):
        ...
        # Front animation
        if not os.path.exists(
            paths['outputs']['figures']['front_animation_2']
        ):
            runtime.plot_fronts(
                paths['outputs']['figures']['front_animation_dir']
            )
            # Requires imagemagick http://www.imagemagick.org/script/download.php  # noqa
            os.system("magick convert -delay 10 -loop 0 io/outputs/figures/front_animation/*.png {}".format(paths['outputs']['figures']['front_animation_2']))  # noqa
